...
public class Bruitage extends Thread {
	...
	try{
	    ...
	    if (fis == null) {
		System.err.println("[Bruitage] Ressource audio introuvable: "+fic);
		return;
	    }
	    try (InputStream audioIn = fis.openStream()){
    ...
    public void run() {
	if (player == null) {
		System.err.println("[Bruitage] Pas de player initialisé, impossible de jouer le son.");
		return;
	}
	try{
	    player.play();
